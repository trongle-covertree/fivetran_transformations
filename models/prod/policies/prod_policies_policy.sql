...
(select
  ...
  sk,
  documents,
  to_timestamp_tz(to_number(created_timestamp)/1000) as created_timestamp,
  to_timestamp_tz(to_number(created_timestamp_dynamo)/1000) as created_timestamp_dynamo,
  to_timestamp_tz(to_number(issued_timestamp)/1000) as issued_timestamp,
  fees,
  characteristics,
  config_version,
  gross_fees,
  payment_schedule_name,
  to_timestamp_tz(original_contract_start_timestamp/1000) as original_contract_start_timestamp,
  to_timestamp_tz(to_number(updated_timestamp)/1000) as updated_timestamp,
  product_name,
  exposures,
  invoices,
  to_timestamp_tz(effective_contract_end_timestamp/1000) as effective_contract_end_timestamp,
  product_locator,
  policyholder_locator,
  gross_fees_currency,
  to_timestamp_tz(original_contract_end_timestamp/1000) as original_contract_end_timestamp,
  display_id,
  modifications,
  locator,
  cancellation,
