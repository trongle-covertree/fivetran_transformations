...
{% if is_incremental() %}
  and (created_timestamp > (select created_timestamp from transformations_dynamodb.prod_policies_policy order by created_timestamp desc limit 1) or updated_timestamp > (select updated_timestamp from transformations_dynamodb.prod_policies_policy order by updated_timestamp desc limit 1))
{% endif %}
