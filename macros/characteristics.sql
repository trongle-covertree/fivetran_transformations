...
{% if characteristics|length > 0 %}
    {% if is_incremental() %}
        {% if pk|length == 1 %}
            {% set delete_query %}
            DELETE FROM {{ env }}.{{ prefix }}_policy_characteristics where PK in {{ pk|replace(",", "") }}
            {% endset %}
    ...
    SELECT Column1 AS PK, Column2 AS QUOTE_INCEPTION_DATE, Column3 AS AUTO_POLICY_WITH_AGENCY, to_date(Column4) AS DATE_OF_BIRTH,
        ...
        Column46 AS INSURANCE_SCORE, to_double(Column47) AS GROSS_PREMIUM, Column48 AS GROSS_PREMIUM_CURRENCY, Column49 AS GROSS_TAXES, Column50 AS GROSS_TAXES_CURRENCY,
    ...
    (
        ...
        {% if char_field_group_keys['agency_contact_name']|length > 0 %}'{{ char_field_group_keys['agency_contact_name'] | replace("'", "\\'") }}'{% else %}null{% endif %},
    ...
    {# fieldValues #}
        ...
        {% if char_json.grossPremium|length > 0 %}'{{ char_json.grossPremium }}'{% else %}null{% endif %},
        '{{ char_json.grossPremiumCurrency or null }}',
        {% if char_json.grossTaxes|length > 0 %}'{{ char_json.grossTaxes }}'{% else %}null{% endif %},
