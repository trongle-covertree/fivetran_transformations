...
{% if exposures|length > 0 %}
    {% if is_incremental() %}
        {% if pk|length == 1 %}
            {% set delete_query %}
            DELETE FROM {{ env }}.{{ prefix }}_policy_exposures_address where PK in {{ pk|replace(",", "") }}
            {% endset %}
            ...
            {% for exposure_json in exposure_arr %}
                ...
                {% for char in exposure_json.characteristics if exposure_json.name != 'Policy Level Coverages' %}
                    ...
                    {% for current_char_key in char.fieldGroupsByLocator.keys() %}
                        {% if 'street_address' in char.fieldGroupsByLocator[current_char_key] and 'previous_country_policyholder' not in char.fieldGroupsByLocator[current_char_key]
                            and 'officer_first_name' not in char.fieldGroupsByLocator[current_char_key] and 'officer_last_name' not in char.fieldGroupsByLocator[current_char_key] and 'officer_mail_address' not in char.fieldGroupsByLocator[current_char_key]
                            and 'account_number' not in char.fieldGroupsByLocator[current_char_key] and 'additional_interest_id' not in char.fieldGroupsByLocator[current_char_key] %}
...
SELECT Column1 AS ID, Column2 AS PK, Column3 AS STREET_ADDRESS, Column4 AS LOT_UNIT, Column5 AS CITY, Column6 AS STATE, Column7 AS ZIP_CODE,
    Column8 AS COUNTY, Column9 AS COUNTRY, to_timestamp(Column10) AS CREATED_TIMESTAMP, to_timestamp(Column11) AS UPDATED_TIMESTAMP,
    to_timestamp(Column12) AS POLICY_CREATED_TIMESTAMP, to_timestamp(Column13) AS POLICY_UPDATED_TIMESTAMP FROM VALUES
    ('NO FIELDS', null, null, null, null, null, null, null, null, null, null, null, null) limit 0
{% endif %}
{% endmacro %}