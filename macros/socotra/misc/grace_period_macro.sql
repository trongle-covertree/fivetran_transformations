...
select
    ...
    to_date(convert_timezone('Etc/GMT', 'America/Los_Angeles', to_timestamp_ntz(cancel_effective_timestamp/1000))) as cancel_effective_date,
...
{% if is_incremental() %}
where ( to_timestamp_tz(created_timestamp/1000) > (select created_timestamp from {{ sf_schema }}.grace_period order by created_timestamp desc limit 1)
    or to_timestamp_tz(datamart_created_timestamp/1000) > (select datamart_created_timestamp from {{ sf_schema }}.grace_period order by datamart_created_timestamp desc limit 1)
    or to_timestamp_tz(datamart_updated_timestamp/1000) > (select datamart_updated_timestamp from {{ sf_schema }}.grace_period order by datamart_updated_timestamp desc limit 1))
