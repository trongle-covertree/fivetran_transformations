...
select
	...
	min(Case when field_name = 'windstorm_or_hail_exclusion' then regexp_replace(field_value, '[$,]')::int End) windstorm_or_hail_exclusion,
	...
	min(Case when field_name = 'secondary_residence_liability_address' then field_value End) secondary_residence_liability_address,
	min(Case when field_name = 'secondary_residence_liability_group' then field_value End) secondary_residence_liability_group,
	min(Case when field_name = 'mine_sub_add_living_expense_limit' then regexp_replace(field_value, '[$,]')::int end) mine_sub_add_living_expense_limit,
	min(Case when field_name = 'mine_subsidence_illinois_other_structures_value' then regexp_replace(field_value, '[$,]')::int end) mine_subsidence_illinois_other_structures_value,
	min(Case when field_name = 'mine_subsidence_deductible_indiana' then regexp_replace(field_value, '[$,]')::int end) mine_subsidence_deductible_indiana,
	min(Case when field_name = 'mine_subsidence_group' then field_value End) mine_subsidence_group,
	min(Case when field_name = 'mine_subsidence_illinois_other_structures_description' then field_value End) mine_subsidence_illinois_other_structures_description,
	min(Case when field_name = 'mine_subsidence_deductible_ohio' then regexp_replace(field_value, '[$,]')::int end) mine_subsidence_deductible_ohio,
	exposure_locator,
