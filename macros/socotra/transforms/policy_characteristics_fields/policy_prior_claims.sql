...
{% if is_incremental() %}
    and (to_timestamp_tz(poc.datamart_created_timestamp/1000) > (select datamart_created_timestamp from {{ sf_schema }}.policy_prior_claims order by datamart_created_timestamp desc limit 1)
      or to_timestamp_tz(poc.datamart_updated_timestamp/1000) > (select datamart_updated_timestamp from {{ sf_schema }}.policy_prior_claims order by datamart_updated_timestamp desc limit 1))
{% endif %}
