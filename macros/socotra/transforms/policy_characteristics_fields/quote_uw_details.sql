...
from {{ socotra_db }}.quote_policy_characteristics_fields as pcf
	...
	{% if is_incremental() %}
		and (to_timestamp_tz(pcf.datamart_created_timestamp/1000) > (select datamart_created_timestamp from {{ sf_schema }}.quote_policy_uw_details order by datamart_created_timestamp desc limit 1)
			or to_timestamp_tz(pcf.datamart_updated_timestamp/1000) > (select datamart_updated_timestamp from {{ sf_schema }}.quote_policy_uw_details order by datamart_updated_timestamp desc limit 1))
	{% endif %}
