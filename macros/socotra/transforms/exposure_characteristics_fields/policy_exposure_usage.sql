...
{% if is_incremental() %}
    and (to_timestamp_tz(ecf.datamart_created_timestamp) > (select datamart_created_timestamp from {{ sf_schema }}.policy_exposure_usage order by datamart_created_timestamp desc limit 1)
      or to_timestamp_tz(ecf.datamart_updated_timestamp) > (select datamart_updated_timestamp from {{ sf_schema }}.policy_exposure_usage order by datamart_updated_timestamp desc limit 1))
{% endif %}
